...
public class BookDao implements BookDAO {
	...
	public int createBook(Book book) {
		...
		int i = jdbcTemplate
				.update("INSERT into Book (name, author, publication, description, noOfCopies) values(?,?,?,?,?)",
						new Object[] { new String(book.getName()),
								new String(book.getAuthor()),
								new String(book.getPublisher()),
								new String(book.getDescription()),
								new Integer(book.getCopies()) });
		System.out
				.println("Successfully inserted the book in the database, return of Update in jdbcTemplate is "
						+ i);
		return i;
	...
	public List<Book> getEnteredBook(Book book) {
		List<Book> bookList = jdbcTemplate
				.query("SELECT bookId,name,author,publication,description,noOfCopies from Book WHERE name=?",
						new Object[] { new String(book.getName()) },
						new BookMapper());
	...
	public List<Book> searchBookById(int bookId) {
		...
		return jdbcTemplate
				.query("SELECT bookId,name,author,publication,description,noOfCopies from Book WHERE bookId=?",
						new Object[] { new Integer(bookId) }, new BookMapper());
	}

	public int updateBookCopies(int bookId, int copies) {
		int additionalCopies = copies;

		List<Book> list = jdbcTemplate
				.query("SELECT bookId,name,author,publication,description,noOfCopies from Book WHERE bookId=?",
						new Object[] { new Integer(bookId) }, new BookMapper());
		int noOfCopies = list.get(0).getCopies();
		int updatedCopies = noOfCopies + additionalCopies;
